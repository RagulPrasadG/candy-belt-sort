...
if ! dpkg -s libgtk-3-0t64 >/dev/null 2>&1 && ! dpkg -s libgtk-3-0 >/dev/null 2>&1; then
  ...
  sudo DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
    ...
    libglu1-mesa libcap2 xvfb libxrandr2 libxcursor1 libxi6 libxinerama1 \
...
xvfb-run -a "$UNITY_BIN" -batchmode -nographics -projectPath "$PROJECT_DIR" \
  -buildTarget "$BUILD_TARGET" \
  -executeMethod CandyBeltSort.CandyBeltMenus.ValidateLevels \
